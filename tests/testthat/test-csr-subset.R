...
test_that("RsparseMatrix subset cols and rows", {
  ...
  expect_equal(m["10", "20", drop = F], as(m_csc["10", "20", drop = F], "RsparseMatrix"))
  expect_equal(m["10", "20", drop = T], m_csc["10", "20", drop = T])
})
...
test_that("RsparseMatrix subset cols", {
  ...
  expect_equal(m[, -1, drop = F], m[, ncol(m), drop = F])
  expect_equal(m[, -1, drop = T], m[, ncol(m), drop = T])
...
test_that("RsparseMatrix subset rows", {
  ...
  expect_equal(m[-1, , drop = T], m[nrow(m), , drop = T])
  expect_equal(m[-1, , drop = T], m[nrow(m), , drop = T])
