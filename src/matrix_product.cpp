...
// implements R's `tcrossprod()`
// FIXME for now works with just double but can be templated in future

// [[Rcpp::export]]
Rcpp::NumericMatrix csr_dense_tcrossprod(const Rcpp::S4& x_csr_r,
                                         const arma::Mat<double>& y_transposed,
                                         int num_threads = 1) {
  const dMappedCSR x_csr = extract_mapped_csr(x_csr_r);
  Rcpp::NumericMatrix res(x_csr.n_rows,
                          y_transposed.n_rows);  // y_transposed.n_rows = y_dense.n_cols
...
// [[Rcpp::export]]
Rcpp::NumericMatrix dense_csc_prod(const Rcpp::NumericMatrix& x_r,
                                   const Rcpp::S4& y_csc_r, int num_threads = 1) {
